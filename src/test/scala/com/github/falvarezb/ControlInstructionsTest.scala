...
class ControlInstructionsTest extends AnyFunSpec with Matchers:
  describe("JSR parser") {
    it("successful parse when operand is an immediate value in decimal representation") {
      ...
      parseJsr(using instructionMetadata, Map.empty[String, InstructionLocation]) shouldBe Right(0x4801)
    }
    ...
    it("successful parse when operand is an immediate value in hex representation") {
      ...
      parseJsr(using instructionMetadata, Map.empty[String, InstructionLocation]) shouldBe Right(0x480a)
    }
    ...
    it("immediate too big") {
      ...
      parseJsr(using instructionMetadata, Map.empty[String, InstructionLocation]) shouldBe Left("ERROR (file - line 1): Immediate operand (2000) out of range (-1024 to 1023)")
    }
    ...
    it("symbolic name not found") {
      val instructionMetadata: InstructionMetadata = InstructionMetadata(LineMetadata("DOES NOT MATTER", List("JSR", "NON_EXISTENT_LABEL"), LineNumber(1), "file"), InstructionLocation(0))
      parseJsr(using instructionMetadata, Map.empty[String, InstructionLocation]) shouldBe Left("ERROR (file - line 1): Symbol not found ('NON_EXISTENT_LABEL')")
    }
  }
