...
import com.github.falvarezb.Parsers.{parseAdd, parseAnd, parseFill, parseJmp, parseJsr, parseJsrr, parseLd, parseLdi, parseLdr, parseLea, parseNot, parseSt, parseSti, parseStr, parseStringz}
...
class ParserTest extends AnyFunSpec with Matchers :
  ...
  describe("LD parser") {
    ...
    it("wrong destination register (DR)") {
      val instructionMetadata = InstructionMetadata(LineMetadata("DOES NOT MATTER", List("LD", "R8", "1"), LineNumber(1)), InstructionLocation(0))
      parseLd(instructionMetadata, Map.empty[String, InstructionLocation]) shouldBe Left("ERROR (line 1): Expected register but found R8")
    }
